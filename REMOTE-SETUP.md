# Agent Watch — 이동용 원격 설정 (여러 Mac + cmux + Tailscale)
...
```bash
cd agent-watch/skill/bridge
npm ci && npm link            # agent-watch 를 PATH에
agent-watch setup             # 훅 등록 + runner 선택 + 기동 + 코드 출력
...
- **페어링 코드:** `agent-watch pair` (회전 코드). 상태/주소는 `agent-watch status`,
  진단은 `agent-watch doctor`.
...
- **삭제:** Macs에서 스와이프. (또는 Mac에서 `agent-watch pair --revoke <id>`)
...
- 아이폰 FaceID/암호 ON (분실 시 승인권한 보호). 분실 기기는
  `agent-watch pair --revoke <id>`로 폐기.
